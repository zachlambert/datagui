...
void LinearLayoutSystem::calculate_size_components(
    Node& node,
    ...
    if (element.length == 0) {
        ...
        while (child_index != -1) {
            ...
            float child_dynamic_a = element.direction == LayoutDirection::Horizontal
                ? child.dynamic_size.x : child.dynamic_size.y;
    ...
    if (element.width == 0) {
        ...
        while (child_index != -1) {
            ...
            float child_dynamic_b = element.direction == LayoutDirection::Horizontal
                ? child.dynamic_size.y : child.dynamic_size.x;
