...
void TextInputSystem::calculate_size_components(Node &node,
...
void TextInputSystem::render(const Node &node, const NodeState &state,
                             Renderers &renderers) const {
  const auto &element = elements[node.element_index];

  renderers.geometry.queue_box(
      Boxf(node.origin, node.origin + node.size), style.text_input.bg_color,
      style.element.border_width,
      state.focused ? style.element.focus_color : style.element.border_color);
  Vecf text_origin = node.origin + Vecf::Constant(style.element.border_width +
                                                  style.element.padding);

  if (state.focused) {
    render_selection(style, font, element.text, element.max_width, text_origin,
                     text_selection, true, renderers.geometry);
  }

  renderers.text.queue_text(font, style.text.font_color, element.text,
                            element.max_width, text_origin);
}
...
bool TextInputSystem::press(const Node &node, const Vecf &mouse_pos) {
  ...
  Vecf text_origin = node.origin + Vecf::Constant(style.element.border_width +
                                                  style.element.padding);
  text_selection.reset(find_cursor(font, element.text, element.max_width,
...
bool TextInputSystem::held(const Node &node, const Vecf &mouse_pos) {
  ...
  Vecf text_origin = node.origin + Vecf::Constant(style.element.border_width +
                                                  style.element.padding);
  text_selection.end = find_cursor(font, element.text, element.max_width,
                                   mouse_pos - text_origin);
  return false;
}
