...
void TextSystem::render(const Node& node, const NodeState& state, Renderers& renderers) const {
  const auto& element = elements[node.element_index];
...
bool TextSystem::press(const Node& node, const Vecf& mouse_pos) {
  const auto& element = elements[node.element_index];
  text_selection.reset(find_cursor(font, element.text, element.max_width, mouse_pos - node.origin));
  return false;
}

bool TextSystem::held(const Node& node, const Vecf& mouse_pos) {
  const auto& element = elements[node.element_index];
  text_selection.end = find_cursor(font, element.text, element.max_width, mouse_pos - node.origin);
  return false;
}
