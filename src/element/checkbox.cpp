...
void CheckboxSystem::calculate_size_components(Node& node, const Tree& tree) const {
  node.fixed_size = Vecf::Constant(font.line_height * style.checkbox.size);
}

void CheckboxSystem::render(const Node& node, const NodeState& state, Renderers& renderers) const {
  ...
  renderers.geometry.queue_box(
      Boxf(node.origin + Vecf::Constant(offset), node.origin + node.size - Vecf::Constant(offset)),
