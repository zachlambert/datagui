...
void CheckboxSystem::calculate_size_components(Node &node,
                                               const Tree &tree) const {
  node.fixed_size = Vecf::Constant(font.line_height * style.checkbox.size);
}

void CheckboxSystem::render(const Node &node, const NodeState &state,
                            Renderers &renderers) const {
  const auto &element = elements[node.element_index];
  const Color &bg_color =
      state.held ? style.element.pressed_bg_color : style.element.bg_color;
  ...
  renderers.geometry.queue_box(Boxf(node.origin, node.origin + node.size),
                               bg_color, style.element.border_width,
                               border_color, 0);
  ...
  renderers.geometry.queue_box(
      Boxf(node.origin + Vecf::Constant(offset),
           node.origin + node.size - Vecf::Constant(offset)),
      style.checkbox.check_color, 0, Color::Black(), 0);
}
