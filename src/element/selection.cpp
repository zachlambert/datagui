...
void SelectionSystem::calculate_size_components(Node &node,
...
void SelectionSystem::render(const Node &node, const NodeState &state,
  ...
  renderers.geometry.queue_box(Boxf(node.origin, node.origin + node.size),
                               ...
                               style.element.border_width, border_color);
  if (element.choice >= 0 && element.choice < element.choices.size()) {
    renderers.text.queue_text(
        font, style.text.font_color, element.choices[element.choice],
        element.max_width,
        node.origin +
