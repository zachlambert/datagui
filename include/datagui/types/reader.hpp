...
public:
  ...
  void integer(datapack::IntType type, void* value) override;
  void floating(datapack::FloatType type, void* value) override;
  bool boolean() override;
  const char* string() override;
  int enumerate(const std::span<const char*>& labels) override;
  std::tuple<const std::uint8_t*, std::size_t> binary(
      std::size_t length,
